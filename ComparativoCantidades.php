...
/***************************************************FILTRO*************************************************/	
	if (isset($_REQUEST['filtro'])) {
		...
		$sql = "SELECT TRAMO, SUBCUENTA, SUM(CANTIDAD) AS SUM_PRESUPUESTO FROM PresupuestoDet WHERE CLASIFICACION = 'CAPEX' AND PERIODO = '".date("Y")."' GROUP BY TRAMO, SUBCUENTA ORDER BY TRAMO";
		// AND SUBCUENTA LIKE '%HORIZONTAL%' AND TRAMO='MARAVATIO - LOS FRESNOS'
	}	
?>
<!doctype html>
<html>
...
<body>
	...
	<div id="contenido">
		...
		while ( odbc_fetch_row($rs) ) {			 
			...
			$sql2 = "SELECT SUM(AvanceDiario.CANTIDAD) AS EJECUTADO FROM PresupuestoDet INNER JOIN AvanceDiario ON PresupuestoDet.TRAMO = AvanceDiario.TRAMO AND PresupuestoDet.CUENTA = AvanceDiario.ACTIVIDAD WHERE (PresupuestoDet.TRAMO = '".$tramo."') AND (PresupuestoDet.PERIODO = '".date("Y")."') AND  (YEAR(AvanceDiario.FECHA) = '".date("Y")."') AND (PresupuestoDet.SUBCUENTA = '".$subcuenta."')";
			//echo $sql2."<br>";
				...
				echo "<tbody>";
				   ...
				   $diferencia = $presupuesto - $ejecutado;
					...
					while ( odbc_fetch_row($rs2) ) { 
						...
						$sql3 = "SELECT SUM(CANTIDAD) EJECUTADO FROM AvanceDiario WHERE (TRAMO = '".$tramo."') AND (ACTIVIDAD = '".$actividad."') AND YEAR(FECHA) = '".date("Y")."'";
						//echo $sql3."<br>";
...
<!-- FILTRO -->
<form action="ComparativoCantidades.php" method="post" enctype="multipart/form-data" >
