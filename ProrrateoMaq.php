...
if (isset($_REQUEST['GuardarCosto'])) { 
	...
	for ($x = 1; $x <= $contador; $x++) {	  
	  if ($_POST["activo".$x.""] <> ""){	
	    ...
	    $noeco = $_POST["noeco".$x.""]; 
	    $base = $_POST["base".$x.""]; 
	    $activo = $_POST["activo".$x.""];
	    $combustible = $_POST["combustible".$x.""];
	    $mantenimiento = $_POST["mantenimiento".$x.""];
	    $otros = $_POST["otros".$x.""];
	    $seguro = $_POST["seguro".$x.""];
	    $llantas = $_POST["llantas".$x.""];
	    $renta = $_POST["renta".$x.""];
	    $bandera = $_POST["bandera".$x.""];
		
		
		if($bandera == ""){
			$sql = "INSERT INTO CostoProrrateoMaq VALUES ('".$noeco."','".$nombre."','".$base."','".$activo."','".$combustible."','".$mantenimiento."','".$renta."','".$llantas."','".$seguro."','".$otros."','".$fecha1."','".$fecha2."','".date("m")."')";
		}else{
			$sql = "UPDATE CostoProrrateoMaq SET ACTIVO_FIJO='".$activo."', COMBUSTIBLE='".$combustible."', MANTENIMIENTO='".$mantenimiento."', RENTA='".$renta."', LLANTAS='".$llantas."', SEGUROS='".$seguro."', OTROS='".$otros."' WHERE NOECO='".$noeco."' AND FECHA_INICIO='".$fecha1."' AND BASE = '".$base."'";
		}	
		//echo $sql;
...
/***************************************************FILTRO*************************************************/	
	if (isset($_REQUEST['filtro'])) {
		...
		if ($baseFiltro == "TODOS"){
			...
			$sql = "SELECT DISTINCT(CvBase) FROM CatMaquinaria WHERE CvBase = '".$baseFiltro."'";		
		}
...
<body>
	...
	<div id="contenido">
		<br>
		<center>
<form action="ProrrateoMaq.php" method="post" enctype="multipart/form-data" >
		<table width="auto" height="auto" border="1" id="tabla">
		  <thead>
		    <tr align="center" class="titulo">
		      <td width="300px">VEHICULO</td>
		      <td width="200px">ACTIVO FIJO</td>
		      <td width="200px">COMBUSTIBLE</td>
		      <td width="200px">MANTENIMIENTO</td>
		      <td width="200px">RENTA</td>
		      <td width="200px">LLANTAS</td>
		      <td width="200px">SEGUROS</td>
		      <td width="200px">OTROS</td>
		    </tr>
		    <tr align="center" class="titulo"><!--2016-09 2016-10-->
		      <td width="300px">RANGO DE FECHAS</td>
		      <td colspan="2"><input type="date" id="fechaIni" name="fechaIni" class="form-control" value="<?php echo date("Y")."-".$mes; ?>-26" readonly></td>	   
		      <td colspan="2"><input type="date" id="fechaFin" name="fechaFin" class="form-control" value="<?php echo date("Y-m"); ?>-25" readonly></td>
		    </tr>
		  </thead>
		  <tbody>  
		<?php 
		...
		$otros = 0;
		$activo_fijo = 0;
		//$sql = "SELECT DISTINCT(TRAMO) FROM AvanceDiario WHERE TRAMO <> ''";
		//$sql = "SELECT DISTINCT(CvBase) FROM CatMaquinaria";
		//echo $sql;
		$rs = odbc_exec( $conn, $sql );
		if ( !$rs ) { 
		...
		while ( odbc_fetch_row($rs) ) {			 
			$base = odbc_result($rs, 'CvBase');
				...
				echo "<tbody>";
				   if ($base1 != $base){
				   echo "<tr class = 'tabla'>
				   			<td align='center' colspan='8'><strong><h4>".$base."</h4></strong></td>
				   		 </tr>";
			$sql2 = "SELECT DISTINCT(NoEco), Descrip FROM CatMaquinaria WHERE CvBase = '".$base."' ORDER BY Descrip";
			//echo $sql2;
			...
			while ( odbc_fetch_row($rs2) ) { 
				$nombre = odbc_result($rs2, 'Descrip');
				$noeco = odbc_result($rs2, 'NoEco');
				$bandera = "";
				$cont++;//2016-09
				
				$sql3 = "SELECT * FROM CostoProrrateoMaq WHERE NOECO='".$noeco."' AND FECHA_INICIO='".date("Y")."-".$mes."-26'";
				//echo $sql3;
				...
				while ( odbc_fetch_row($rs3) ) { 
					$activo_fijo = odbc_result($rs3, 'ACTIVO_FIJO');
					$combustible = odbc_result($rs3, 'COMBUSTIBLE');
					$mtto = odbc_result($rs3, 'MANTENIMIENTO');
					$renta = odbc_result($rs3, 'RENTA');
					$llantas = odbc_result($rs3, 'LLANTAS');
					$seguros = odbc_result($rs3, 'SEGUROS');
					$otros = odbc_result($rs3, 'OTROS');
					$bandera = "Si";
				}
				
				echo "<tr class = 'tabla'>
						<td><input type='hidden' id='nombre".$cont."' name='nombre".$cont."' value='".$nombre."'>".$nombre."</td>
						<td align='right'><input type='number' id='activo".$cont."' name='activo".$cont."' class='form-control' onblur='Importe()' value='".$activo_fijo."' step='0.01'><input type='hidden' id='noeco".$cont."' name='noeco".$cont."' value='".$noeco."'><input type='hidden' id='base".$cont."' name='base".$cont."' value='".$base."'><input type='hidden' id='bandera".$cont."' name='bandera".$cont."' value='".$bandera."'></td>
						<td align='right'><input type='number' id='combustible".$cont."' name='combustible".$cont."' class='form-control' onblur='Importe()' value='".$combustible."' step='0.01'></td>
						<td align='right'><input type='number' id='mantenimiento".$cont."' name='mantenimiento".$cont."' class='form-control' onblur='Importe()' value='".$mtto."' step='0.01'></td>
						<td align='right'><input type='number' id='renta".$cont."' name='renta".$cont."' class='form-control' onblur='Importe()' value='".$renta."' step='0.01'></td>
						<td align='right'><input type='number' id='llantas".$cont."' name='llantas".$cont."' class='form-control' onblur='Importe()' value='".$llantas."' step='0.01'></td>
						<td align='right'><input type='number' id='seguro".$cont."' name='seguro".$cont."' class='form-control' onblur='Importe()' value='".$seguros."' step='0.01'></td>
						<td align='right'><input type='number' id='otros".$cont."' name='otros".$cont."' class='form-control' onblur='Importe()' value='".$otros."' step='0.01'></td>
					</tr>
				</tbody>";
				$base1 = $base;
				 }
		...
		/*echo "<tr class='tabla'>
				<td align='center'>TOTAL</td><td><input type='text' name='totalO' id='totalO' class='form-control' readonly></td>
				<td><input type='text' name='totalE' id='totalE' class='form-control' readonly></td>
				<td><input type='text' name='totalE' id='totalE' class='form-control' readonly></td>
				<td><input type='text' name='totalE' id='totalE' class='form-control' readonly></td>
				<td><input type='text' name='totalE' id='totalE' class='form-control' readonly></td>
				<td><input type='text' name='totalE' id='totalE' class='form-control' readonly></td>
        ...
        <tfoot>
        	<tr>
            	<td colspan="8" align="center"><button type="submit" class="btn btn-primary btn-lg" name="GuardarCosto" id="GuardarCosto">Guardar</button></td>
            </tr>
        </tfoot>
	</table>
</form>
