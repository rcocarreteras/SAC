...
if ($_SESSION['S_Base'] == "EN01','EN01','OC01','OC01','USA01','USA01','OC01','OC01','PA01','PA01','ZI01','ZI01','JA01','TE01','TO01','TO01"){
		$mostrar="Todas";
}else{
...
$buscarbase = $_SESSION['S_Base'];
	switch($buscarbase){
		case "EN01":
			$buscarbase = "LEON','EN01";
		break;
		case "TE01":
			$buscarbase = "TEPATITLAN','TE01";
		break;
		case "TO01":
			$buscarbase = "TONALA','TO01";
		break;
		case "ZI01":
			$buscarbase = "ZINAPECUAR','ZI01";
		break;
		case "PA01":
			$buscarbase = "PANINDICUA','PA01";
		break;
		case "OC01":
			$buscarbase = "OCOTLAN','OC01";
		break;
		case "JA01":
			$buscarbase = "TEPATITLAN','TE01";
		break;
		case "USA01":
			$buscarbase = "LEON','USA01";
		break;
		case "SB1":
			$buscarbase = "SANBLAS','SB01";
		break;
	}
...
if ($buscarbase == "EN01','EN01','OC01','OC01','USA01','USA01','OC01','OC01','PA01','PA01','ZI01','ZI01','JA01','TE01','TO01','TO01"){
		$mostrar="Todas";
	}else{
		$mostrar = $buscarbase;
	}
...
//	if ($buscarbase == "ENCARNACION','EN01"){
...
//	}else{
		$sql1 = "SELECT CvIns as Articulo, Descrip, Unid as Unidad, CASE WHEN SUM(Cant) = 0 THEN 0 ELSE ROUND((SUM(Importe)/SUM(Cant)), 2) END as Precio_Unitario, SUM(Cant) as Exist, SUM(ISNULL(Importe,0)) * SUM(Cant) as Importe, '' as Observaciones, '' as Presupuesto, 'Oracle' as Ubicacion_almacen from MovAlmac where base IN ('".$buscarbase."') AND Importe > 0 GROUP BY CvIns, Descrip, Unid ORDER BY Ubicacion_almacen";
		$sql = "select SUM(Importe) TOTAL from MovAlmac where base IN ('".$buscarbase."')";
  		$sql3 = "select * from Almacen where Base in ('".$buscarbase."') AND Ubicacion_almacen='Sac' order by Ubicacion_almacen";
//	}

	$sql2 = "select * from Salidas where Base in ('".$real."') and FECHA = '".date("Y-m-d")."'";
	...
	$sql1 = "SELECT CvIns as Articulo, Descrip, Unid as Unidad, CASE WHEN SUM(Cant) = 0 THEN 0 ELSE ROUND((SUM(Importe)/SUM(Cant)), 2) END as Precio_Unitario, SUM(Cant) as Exist, SUM(ISNULL(Importe,0)) * SUM(Cant) as Importe, '' as Observaciones, '' as Presupuesto, 'Oracle' as Ubicacion_almacen from MovAlmac where base in ('".$_SESSION['S_Base']."','".$_SESSION['S_Plaza']."') AND Importe > 0 GROUP BY CvIns, Descrip, Unid ORDER BY Ubicacion_almacen";
	$sql = "select SUM(Importe) TOTAL from MovAlmac where base in ('".$_SESSION['S_Base']."','".$_SESSION['S_Plaza']."')";
...
##########################################EJECUTAR ACTUALIZACIÓN DE ALMACEN ORACLE
#EXECUTE uspCargaAlmacOra 'OCT-17'  
...
$(document).ready(function () {
            ...
            $("#buscarArt").keypress(function(e){
            	if(e.which == 13){
					...
					var base = '<?php echo $var[0]; ?>';
				
					$("#buscarArt").val('');					
					$.post("consultaAvanceDiario.php", { art1: articulo, Base: base }, function(data){
			...
			if(cont == 0){
				...
				$.post("consultaAvanceDiario.php", { FolioSalida: '', Base: base }, function(data){
					...
					for (var i = 0; i < cont; i++) {
						...
						$.post("consultaAvanceDiario.php", { GuardaSalida: '', FolioSa: folio, Mostrar: mostrar, Base: base, Concepto: arrayconcep[i], cantidad: cant[i], Importe: precio[i] }, function(data2){
						//alert(data2);	
...
<div class="contenedor">
	...
	<div class="menulateral">
		...
		<?php if ($_SESSION['S_Privilegios'] == "COORDINADOR" || $_SESSION['S_Privilegios'] == "ADMINISTRADOR"){ ?>
          ...
          <?php } ?>
		 <!--<a href="#" data-toggle="modal" data-target=".actualizar" onClick="Limpiar()"><div class="submenu_lateral" id="sub5">
			<span class="glyphicon glyphicon-upload"></span> &rlm; Actualizar Oracle
		</div> </a>-->
 </form>          
	</div>
...
<!--ACTUALIZAR
<form id="formulario5" action="Almacen1.php" method="post" enctype="multipart/form-data" >
<div class="modal fade" id="descargar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Almac&eacute;n Oracle</h4>
      </div>
      <div class="modal-body">
	<input type="hidden" name="fol_desc" id="fol_desc">
        <center>&iquest;Deseas actu?</center>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
        <button type="button" class="btn btn-primary" data-dismiss="modal" id="Descargar">Descargar</button>
  ...
  <tr>
    ...
    <td width="251">
    	<select class="form-control" name="buscarbase" id="buscarbase" style="width:200px" >
			...
			<!--<option value="EN01','EN01','OC01','USA01','USA01','OC01','OC01','PA01','PA01','ZI01','ZI01','SB1','PA01','JA01','TE01','TO01','TO01">Todas</option>-->				
			<?php	
			}else{
			  $i=1;
			  $selected="";
