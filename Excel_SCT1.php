		...
		->setCellValue('B4', utf8_encode("Subtramo: ".$subtramo));	
				...
				for ($i=4; $i < 97; $i++) {
					...
					for ($j=1; $j <= 12 ; $j++) { 
						...
						$sql2 = "SELECT ISNULL(SUM(PresupuestoOpex.CANTIDAD), 0) CANTIDAD, PrecioSCT.UNIDAD, PrecioSCT.PRECIO_UNITARIO FROM PresupuestoOpex INNER JOIN PrecioSCT ON PresupuestoOpex.ACTIVIDAD = PrecioSCT. CONCEPTO WHERE PrecioSCT.CLAVE_SCT = '".trim($clave)."' AND PERIODO = '".$year.$j."' AND TRAMO='".$tramo."' GROUP BY PrecioSCT.UNIDAD, PrecioSCT.PRECIO_UNITARIO";
						$rs2 = odbc_exec( $conn2, $sql2);
						if ( !$rs2 ) { 
							exit( "Error en la consulta SQL" );
						}
						...
						$sql2 = "SELECT ISNULL(SUM(IMPORTE), 0) IMPORTE FROM PRESUPUESTOOPEX WHERE PERIODO = '".$year.$j."' AND TRAMO='".$tramo."'";
						$rs2 = odbc_exec( $conn2, $sql2);
						if ( !$rs2 ) {
							exit( "Error en la consulta SQL" );
						}
					...
					$sql2 = "SELECT ISNULL(SUM(AvanceDiario.CANTIDAD), 0) CANTIDAD, PrecioSCT.UNIDAD, PrecioSCT.PRECIO_UNITARIO FROM AvanceDiario INNER JOIN PrecioSCT ON AvanceDiario.ACTIVIDAD = PrecioSCT. CONCEPTO WHERE PrecioSCT.CLAVE_SCT = '".trim($clave)."' AND YEAR(FECHA) = '".$year."' AND MONTH(FECHA) = '".$month."' AND TRAMO='".$tramo."' GROUP BY PrecioSCT.UNIDAD, PrecioSCT.PRECIO_UNITARIO";
					$rs2 = odbc_exec( $conn2, $sql2);
					if ( !$rs2 ) {
						exit( "Error en la consulta SQL" );
					}
					...
					$sql2 = "SELECT ISNULL(SUM(PresupuestoOpex.CANTIDAD), 0) CANTIDAD, PrecioSCT.UNIDAD, PrecioSCT.PRECIO_UNITARIO FROM PresupuestoOpex INNER JOIN PrecioSCT ON PresupuestoOpex.ACTIVIDAD = PrecioSCT. CONCEPTO WHERE PrecioSCT.CLAVE_SCT = '".trim($clave)."' AND PERIODO = '".$year.$month."' AND TRAMO='".$tramo."' GROUP BY PrecioSCT.UNIDAD, PrecioSCT.PRECIO_UNITARIO";
					$rs2 = odbc_exec( $conn2, $sql2);
					if ( !$rs2 ) { 
						exit( "Error en la consulta SQL" );
					}
...
/*$dsv=array(
	new \PHPExcel_Chart_DataSeriesValues('String', 'Datos!$E$7', NULL, 1),
	new \PHPExcel_Chart_DataSeriesValues('Number', 'Datos!$E$8:$E$20', NULL, 90),
);
...
$title = new \PHPExcel_Chart_Title('Grafico');
